...
> 文档状态：`implemented_and_production_verified`
> 版本：`1.0.3`
> 日期：`2026-07-22`
> 首次生产验收：`2026-07-23`
...
- [x] 用测试 ref 验证 workflow 可创建、exact-OID 快进更新、被 GitHub 明确拒绝非快进强推，并以精确 lease 删除未验收 ref；
- [x] 对 `.github/workflows/**` 设置 CODEOWNERS 或等价审核规则；
- [x] 在 registry 中登记至少一个点播候选和一个直播候选；
- [x] README 明确标注 `public_unverified` 含义、风险和下架入口；
- [x] 用户于 `2026-07-23` 明确豁免真实目标客户端人工验收；本次上线以裸 Raw、客户端同构 API 请求和真实 HLS 媒体分片闭环为验收边界；
- [x] 首次发布后保存三个稳定 Raw URL 和 commit SHA 证据：
  ...
  - 单配置直连：`https://raw.githubusercontent.com/azhansy/ds-tvbox/generated/dist/configs/stable.json`；
  - `generated` HEAD：`44c4eabe9980ef2e8c99e836465b1e01c57b4ead`；内容提交：`727e41369c59dddc58395ce6a24bc90cf233e9c1`；
  - 首次成功发布：[GitHub Actions run 29974298927](https://github.com/azhansy/ds-tvbox/actions/runs/29974298927)。
